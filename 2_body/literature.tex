...
The measurement of one of the entangled qubits has collapsed the superposition for both qubits. This surprising result was initially deemed unphysical on the grounds that it violates local realism by Einstein, Poldowsky, and Rosen, the first to describe this behaviour\cite{epr_paradox}. Nevertheless, quantum entanglement and its implications have been repeatedly verified through experimentally violating Bell's inequality, establishing that the correlations produced from quantum entanglement cannot be explained by local hidden variables.\todo{citation here for Bell's inequality and experiments violating it}
...
Approximate algorithms are the focus of this thesis.\todo{expand on this}
...
\begin{itemize}
    \item \textit{Graph Edit Distance (GED):} Counts how many `edits' are required to transform one graph into another, where `edits' include inserting/deleting a vertex or edge, or substiting one vertex or edge with another. The lower the distance, the more similar the two graphs are.\todo{citations for each of these}
...
\subsection{QAOA}\todo{finish this section}
...
Here, $\sigma_j^x$ denotes the $j$th Pauli X operator.\todo{that doesn't seem right}
...
In his work, Bennett provides some examples of mixing graphs with known efficient implementations on a quantum computer:\todo{do I cite it again here?}
...
The choice of mixing graph directly determines how probability amplitudes are propagated, making it possible to exploit problem structure, unlike Grover's diffusion operator or the complete-graph mixer, which treat all states as equally connected.\todo{citations}
